  ...
  async uploadFile(req, res) {
    try {
      ...
      const response = await s3
        ...
        .promise();
      const objectUrl = `https://${process.env.AWS_BUCKET_FOR_PDF}.s3.${process.env.AWS_REGION}.amazonaws.com/uploads/${files.originalname}`;
      console.log(response);

      return res.status(200).json({ error: false, response, url: objectUrl });
    } catch (error) {
